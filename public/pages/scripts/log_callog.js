$(function () {
    ...
    var table = $("#sample_1").DataTable({
        ...
        "order": [[4, "desc"]], // Sort by sendtime column (5th column, index 4) descending
        "columnDefs": [{
            "targets": 0,
            "orderable": false,
            "searchable": false
        }, {
            "targets": 7, // Delete column
            "orderable": false,
            "searchable": false
        }, {
            "searchable": true,
...
{
    var id = $("#deleteCallId").text();
    if (id == -1)
        return;
    $.ajax({
        type: 'POST',
        url: baseUrl + 'log/deleteLog',
        data: {
            'ID' : id,
        },
        dataType: 'html',
        success: function(res){
            window.location.href = baseUrl + "/log/calllog";        
        },
        error: function() {
...
var onSearchLog = function(){
    ...
    location.href = baseUrl + "log/calllog/from="+ sentFrom +"/to="+ sentTo +"/snd="+ sender +"/rcvd="+ receiver;
...
$(document).ready(function () {
    ...
    $("#deletelog").click(function(){
        ...
        if (flag == 0){
            //alert("삭제하려는 호출리력을 선택하십시오.");
            location.href = location.href;
        }
        else
            $("#deleteLogMessage").text("Do you want to delete the selected call logs?");
    });
